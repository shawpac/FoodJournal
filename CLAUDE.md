...
- `Views/HealthMetricsView.swift` (v2.2) — the **Health Data tab's** root. LazyVGrid of tiles for sleep + BP specials + 9 generic vital tiles; tapping a tile pushes `MetricTrendView` (generic over `HealthMetric`) or the bespoke `SleepTrendView` / `BPTrendView`. 7d/30d range picker on each trend page. Swift Charts. Missing days are gaps in the chart, never zero values. Footer disclaimer makes the denial-vs-no-data ambiguity explicit. Owns its own NavigationStack since it's a tab destination.
